...
@section('page-header')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto">{{ __('menu.security') }}</h4>
    ...
    <div class="row">
        ...
        @if ($id == 1)
            <div class="card w-100">
                <div class="card-body">
                    <h5 class="card-title mb-3">{{ __('lable.employees') }}</h5>
                    @if (!count($corona))
        ...
        @else
            <div class="card  w-100">
                <div class="card-body">
                    <h5 class="card-title mb-3">{{ __('lable.visitors') }}</h5>
                    @if (!count($visitors))
                        {{ session()->put('error', __('msg.noDataToday')) }}
                        @include('alerts.error')
                    @else
                                ...
                                <tbody>
                                    @foreach ($visitors as $item)
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{ route('visitors.edit', $item->id) }}"
                                                    class="btn btn-primary">السماح بالدخول</a>
                                            </td>
                                        </tr>
                                    @endforeach
